...
def create_professional_patient():
    ...
    try:
        ...
        try:
            ...
            worksheet.append_row(nuevo_paciente_row)
            logger.info(
...
def search_enhanced():
    ...
    try:
        ...
        # Búsqueda real combinando PubMed + Europe PMC con normalización y deduplicación
        from medical_apis_integration import MedicalAPIsIntegration
        import re

        apis = MedicalAPIsIntegration()

        pub = apis.buscar_tratamiento_pubmed(motivo_consulta, "general", None) or []
        epmc = apis.buscar_europepmc(motivo_consulta, "general", None) or []
        all_items = pub + epmc
        ...
        norm = []
        for t in all_items:
            doi = getattr(t, "doi", None)
            fuente = (getattr(t, "fuente", "") or "").title()
            titulo = getattr(t, "titulo", "")
            resumen = getattr(t, "resumen", "")
            fecha = getattr(t, "fecha_publicacion", "")
            year = getattr(t, "año_publicacion", None) or year_from(fecha)
            tipo = getattr(t, "tipo_evidencia", "")
            url = f"https://doi.org/{doi}" if doi and doi != "Sin DOI" else ""
            norm.append(
                {
                    "titulo": titulo,
                    "resumen": resumen,
                    "doi": doi if doi and doi != "Sin DOI" else None,
                    "fuente": fuente,
...
def analyze_enhanced():
    """Análisis mejorado para el frontend (sidebar elegante).
    Devuelve un objeto 'analisis_mejorado' con claves esperadas por la UI.
    """
    try:
        data = request.get_json(force=True) or {}
        motivo = (data.get("motivo_consulta") or "").strip()
        ...
        if not motivo:
            return (
                jsonify({"success": False, "message": "motivo_consulta es requerido"}),
                400,
            )

        # Señales simples desde el texto para enriquecer el análisis
        ...
        # Palabras clave simuladas con intensidades
        palabras = [w for w in motivo_lower.replace(",", " ").split() if len(w) > 3]
        unicas = []
        for w in palabras:
            if w not in unicas:
                unicas.append(w)
        palabras_clave_identificadas = [
            {"palabra": p, "intensidad": min(1.0, 0.4 + 0.05 * i)}
            for i, p in enumerate(unicas[:6])
        ]

        # Buscar evidencia (PubMed + Europe PMC + API3 opcional) y normalizar
        evidencia_items = []
        try:
            from medical_apis_integration import MedicalAPIsIntegration
            import re
            ...
            resultados_pub = (
                apis.buscar_tratamiento_pubmed(motivo, tipo_atencion or "general", edad)
                or []
            )
            resultados_epmc = (
                apis.buscar_europepmc(motivo, tipo_atencion or "general", edad) or []
            )
            resultados_api3 = []  # Placeholder si existe una tercera fuente
            ...
            # Unificar
            for t in resultados_pub + resultados_epmc + resultados_api3:
                doi = getattr(t, "doi", None)
                fuente = (getattr(t, "fuente", "") or "").title()
                titulo = getattr(t, "titulo", "")
                resumen = getattr(t, "resumen", "")
                fecha_pub = getattr(t, "fecha_publicacion", "")
                year = getattr(t, "año_publicacion", None) or extraer_year(fecha_pub)
                tipo_estudio = getattr(t, "tipo_evidencia", "")
                url = f"https://doi.org/{doi}" if (doi and doi != "Sin DOI") else ""
            ...
            # Deduplicar por DOI luego por título
            vistos = set()
            unicos = []
            for ev in evidencia_items:
                key = ev.get("doi") or (ev.get("titulo") or "").lower().strip()
                if key in vistos:
                    continue
                vistos.add(key)
                unicos.append(ev)
            evidencia_items = unicos[:5]
        except Exception:
            evidencia_items = []

        patologias_sugeridas = [
            {"nombre": "Lesión de tejidos blandos", "confianza": 0.6},
            {"nombre": "Dolor inespecífico", "confianza": 0.55},
        ]
        escalas_recomendadas = [
            {"nombre": "EVA", "descripcion": "Escala Visual Análoga del dolor"},
            {"nombre": "LEFS", "descripcion": "Lower Extremity Functional Scale"},
        ]
        recomendaciones = [
            "Aplicar protocolo PRICE en fase aguda si corresponde",
            "Progresar a ejercicios funcionales supervisados",
            "Reevaluar en 1-2 semanas",
        ]

        analisis_mejorado = {
            "palabras_clave_identificadas": palabras_clave_identificadas,
            "region_anatomica": region,
            "patologias_sugeridas": patologias_sugeridas,
            "escalas_recomendadas": escalas_recomendadas,
            "evidencia_cientifica": evidencia_items,
            "recomendaciones": recomendaciones,
        }

        return jsonify({"success": True, "analisis_mejorado": analisis_mejorado})
