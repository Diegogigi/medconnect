...
try:
    ...
    from flask import Flask, render_template, request, jsonify, session, redirect, url_for, flash, make_response, send_from_directory, send_file, abort, Response
...
# Asegurar que CORS esté configurado
try:
    from flask_cors import CORS
    CORS(app)
    print('✅ CORS configurado correctamente')
except Exception as e:
    print(f'⚠️ CORS no disponible: {e}')
...
# Estas rutas DEBEN estar aquí para registrarse correctamente

@app.route('/')
def index():
    """Página principal - Landing page"""
    try:
        # Obtener datos básicos del usuario si está logueado
    ...
    try:
        ...
        if 'user_id' in session:
            ...
            if user_type == 'profesional':
                ...
                return redirect('/patient')  # URL directa para evitar problemas
        ...
        # Placeholder para autenticación real
        return render_template('login.html', 
                             message='Funcionalidad de login implementada', 
                             success=True)
...
def register():
    ...
    try:
        ...
        return render_template('register.html', 
                             message='Funcionalidad de registro implementada', 
                             success=True)
        
    except Exception as e:
        print(f'[ERROR] Error en register: {e}')
        return render_template('register.html', 
                             message='Error interno del servidor', 
                             success=False)
...
def patient_dashboard():
    ...
    try:
        ...
        print(f'[ERROR] Error en patient dashboard: {e}')
        return redirect(url_for('login'))
...
def professional_dashboard():
    ...
    try:
        user_data = session.get('user_data', {})
        return render_template('professional.html', user=user_data)
    except Exception as e:
        print(f'[ERROR] Error en professional dashboard: {e}')
        return redirect(url_for('login'))
...
try:
    ...
    if json_content:
        ...
        try:
            ...
            logger.info(f"[OK] JSON v lido, proyecto: {test_json.get('project_id', 'N/A')}")
...
def logout():
    ...
    try:
        ...
        logger.info("[ACTUALIZAR] Redirigiendo a p gina principal con headers anti-cache...")
        return response
...
def index():
    ...
    try:
        ...
        if logout_param in ['success', 'error']:
            logger.info(f"[ACTUALIZAR] Detectado logout: {logout_param} - Forzando limpieza de sesi n")
            ...
            user_name = None
            user_type = None
            logger.info(f"[OK] Sesi n limpiada por logout: {logout_param}")
        else:
        ...
        logger.info(f"[BUSCAR] Index - user_id: {user_id}, user_name: {user_name}, user_type: {user_type}")
        ...
        logger.error(f"[ERROR] Error en index: {e}")
        return render_template('index.html', user={})

def login():
    """Página de inicio de sesión"""
    logger.info("[BUSCAR] Accediendo a página de login...")
    ...
    if 'user_id' in session:
        user_type = session.get('user_type', 'paciente')
        logger.info(f"[ACTUALIZAR] Usuario ya logueado, redirigiendo a dashboard: {user_type}")
        if user_type == 'profesional':
            return redirect(url_for('professional_dashboard'))
        else:
...
def patient_dashboard():
    ...
    try:
        ...
        # Asegurar que es un paciente
        if user_data.get('tipo_usuario') != 'paciente':
            return redirect(url_for('professional_dashboard'))
        
        return render_template('patient.html', user=user_data)
        
    except Exception as e:
        logger.error(f"[ERROR] Error en patient dashboard: {e}")
        return redirect(url_for('login'))
    ...
    try:
        ...
        if profesional_id:
            ...
            if professional_data:
                ...
                logger.info(f"[BUSCAR] Datos despu s de actualizar con professional_data: {user_data}")
                ...
                if not user_data['genero']:
                    ...
                    logger.info(f"[BUSCAR] G nero inferido del nombre: {user_data['genero']}")
                ...
                logger.info(f"[BUSCAR] Profesi n con g nero: {user_data['profesion_gendered']}")
                
                # Actualizar la sesi n con los datos actualizados
                session['user_data'] = user_data
                logger.info(f"[BUSCAR] Sesi n actualizada con nuevos datos: {session['user_data']}")
...
def get_patient_consultations(patient_id):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"[BUSCAR] Consultas encontradas para paciente {patient_id}: {len(consultations)}")
...
def get_patient_medications(patient_id):
    ...
    try:
        ...
        try:
            ...
            if len(all_values) > 1:
                headers = all_values[0]
                logger.info(f"[LISTA] Headers de Medicamentos: {headers}")
            ...
            logger.info(f"[BUSCAR] Medicamentos encontrados para paciente {patient_id}: {len(medications)}")
...
def get_patient_exams(patient_id):
    ...
    try:
        ...
        try:
                ...
                for row in all_exam_values[1:]:
                    if len(row) >= len(headers) and any(cell.strip() for cell in row):
                        ...
                        if str(patient_id_cell) == str(patient_id):
                            ...
                            logger.info(f"[CALENDARIO] Fecha original: '{original_date}'   Convertida: '{converted_date}'")
            ...
            logger.info(f"[BUSCAR] Ex menes encontrados para paciente {patient_id}: {len(patient_exams)}")
            
            if patient_exams:
                return jsonify({'exams': patient_exams})
                
        except gspread.WorksheetNotFound:
            logger.info("[NOTA] Hoja 'Examenes' no encontrada, intentando con estructura antigua")
        ...
        try:
            ...
            for record in all_records:
                if str(record.get('patient_id', '')) == str(patient_id):
                    ...
                    converted_date = convert_date_format(original_date)
                    logger.info(f"[CALENDARIO] Fecha original (antigua): '{original_date}'   Convertida: '{converted_date}'")
            ...
            logger.info(f"[BUSCAR] Ex menes encontrados en estructura antigua para paciente {patient_id}: {len(patient_exams)}")
...
def get_patient_family(patient_id):
    ...
    try:
        ...
        logger.info(f"[BUSCAR] Familiares encontrados para paciente {patient_id}: {len(patient_family)}")
    ...
    try:
        ...
        try:
            ...
            if row_to_delete:
                ...
                logger.info(f"[OK] Consulta {consultation_id} eliminada para paciente {patient_id}")
...
def delete_medication(patient_id, medication_id):
    ...
    try:
        ...
        try:
            ...
            if row_to_delete:
                worksheet.delete_rows(row_to_delete)
                logger.info(f"[OK] Medicamento {medication_id} eliminado para paciente {patient_id}")
...
def delete_exam(patient_id, exam_id):
    ...
    try:
        ...
        try:
            ...
            if row_to_delete:
                worksheet.delete_rows(row_to_delete)
                logger.info(f"[OK] Examen {exam_id} eliminado para paciente {patient_id}")
...
def delete_family_member(patient_id, family_id):
    ...
    try:
        ...
        if row_to_delete:
            worksheet.delete_rows(row_to_delete)
            logger.info(f"[OK] Familiar {family_id} eliminado para paciente {patient_id}")
...
def update_notification_settings():
    ...
    try:
        ...
        logger.info(f"[OK] Configuraciones de notificaci n actualizadas para usuario {user_id}")
...
INTENT_KEYWORDS = {
    ...
    'paciente_profesional': ['paciente', 'historial paciente', 'datos paciente', 'informaci n paciente'],
            ...
            if spreadsheet:
                try:
                    ...
                    if pacientes_profesional:
                        response = f"[PACIENTES] **Pacientes del Dr(a). {user_name}**\n\n"
...
def health_check():
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"[ADVERTENCIA] No se pudieron obtener estad sticas de API: {e}")
...
def serve_static(filename):
    ...
    try:
        ...
        logger.info(f"[ARCHIVO] Rutas probadas: {[os.path.join(p, filename) for p in static_paths]}")
        ...
        if file_path and os.path.exists(file_path):
            ...
            if file_size > 1024 * 1024:  # Más de 1MB
                logger.info(f"[STREAMING] Sirviendo archivo grande: {filename} ({file_size} bytes)")
            ...
            logger.info(f"[OK] Archivo servido exitosamente: {filename} (tipo: {mimetype}, tamaño: {file_size}) desde {used_path}")
            return response
        else:
            logger.error(f"[ERROR] Archivo no encontrado: {filename}")
            logger.error(f"[ERROR] Rutas probadas: {[os.path.join(p, filename) for p in static_paths]}")
            ...
            if filename.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp')):
                ...
                if os.path.exists(default_image):
                    logger.info(f"[ACTUALIZAR] Sirviendo imagen por defecto para: {filename}")
        ...
        if file and allowed_file(file.filename):
            ...
            try:
                ...
                if exam_row:
                    ...
                    logger.info(f"[OK] Archivo agregado al examen {exam_id}: {filename}")
                    logger.info(f"[ARCHIVO] URLs de archivos actualizadas: {updated_file_urls}")
        ...
        for bot_user in bot_users:
            ...
            if bot_telegram_id:
                ...
                if matching_web_user:
                    ...
                    logger.info(f"[OK] Usuario ya vinculado: {matching_web_user.get('nombre')} con telegram_id {bot_telegram_id}")
                else:
                    # Reportar usuario del bot sin vincular
                    results['duplicates_found'] += 1
                    logger.info(f"[ADVERTENCIA] Usuario del bot sin vincular: {bot_user_id} con telegram_id {bot_telegram_id}")
...
def link_telegram():
    ...
    try:
        ...
        try:
            ...
            users_worksheet = spreadsheet.worksheet('Usuarios')
            all_records = users_worksheet.get_all_records()
            logger.info(f"[ESTADISTICAS] Total de registros de usuarios: {len(all_records)}")
            ...
            if user_row:
                ...
                if 'telegram_id' in headers:
                    ...
                    logger.info(f"[OK] Columna telegram_id encontrada en posici n: {telegram_col}")
                else:
                    ...
                    logger.info(f"[OK] Columna telegram_id agregada en posici n: {telegram_col}")
                
                logger.info(f"[GUARDAR] Actualizando telegram_id en fila {user_row}, columna {telegram_col}...")
                # Actualizar el telegram_id del usuario
                users_worksheet.update_cell(user_row, telegram_col, telegram_id)
                
                logger.info(f"[OK] Usuario {user_id} ({user_name}) vinculado con Telegram ID: {telegram_id}")
...
<i> Gracias por usar MedConnect!</i> [CORAZON]"""
                ...
                try:
                    ...
                    logger.info(f"[ESTADISTICAS] Ex menes encontrados: {exams_found}, Bot users: {len(bot_user_ids)}")
                ...
                logger.error(f"[ERROR] Usuario {user_id} no encontrado en la hoja de Usuarios")
                return jsonify({'error': 'Usuario no encontrado'}), 404
...
**Formato correcto:** `/codigo MED123456`"""
        ...
        if success and updated_user:
            ...
            logger.info(f"[OK] Cuenta vinculada: Usuario {user_id} con Telegram {telegram_user_id}")
...
def unlink_telegram():
    ...
    try:
        ...
        if success:
            logger.info(f"[OK] Telegram desvinculado para usuario {user_id}")
            return jsonify({
                'success': True,
        ...
        logger.error(f"[ERROR] Error desvinculando Telegram: {e}")
        return jsonify({'error': 'Error interno del servidor'}), 500
...
def update_professional_profile():
    ...
    try:
        ...
        logger.info(f"[ACTUALIZAR] Actualizando perfil profesional - Usuario: {user_id}, Datos: {form_data}")
...
def register_atencion():
    ...
    try:
        ...
        if atencion_id and archivos:
            logger.info(f"[ARCHIVO] Procesando {len(archivos)} archivos para atenci n {atencion_id}...")
            ...
            for archivo in archivos:
                if archivo and archivo.filename:
                    try:
                        ...
                        logger.error(f"[ERROR] Error procesando archivo {archivo.filename}: {e}")
                        continue
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"[ADVERTENCIA] Hoja Atenciones_Medicas no encontrada, creando... Error: {e}")
                # Crear la hoja si no existe
                headers = ['atencion_id', 'profesional_id', 'profesional_nombre', 'paciente_id', 
                          ...
                          'tiene_archivos']
                worksheet = spreadsheet.add_worksheet(title='Atenciones_Medicas', rows=1000, cols=len(headers))
            ...
            if records is None:
                logger.error("[ERROR] No se pudieron obtener registros despu s de reintentos")
            ...
            for i, record in enumerate(records):
                ...
                logger.info(f"[BUSCAR] Registro {i+1}: profesional_id='{record_profesional_id}', buscando='{profesional_id}'")
                
                if record_profesional_id == str(profesional_id):
                    logger.info(f"[OK] Atenci n encontrada para el profesional: {record.get('atencion_id', '')}")
            ...
            logger.info(f"[ESTADISTICAS] Total atenciones del profesional: {len(atenciones_profesional)}")
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"[ADVERTENCIA] Hoja Pacientes_Profesional no encontrada, creando... Error: {e}")
                ...
                if str(record.get('profesional_id', '')) == str(profesional_id):
                    ...
                    for field in string_fields:
                        ...
                        if value is not None:
                            ...
                            paciente_data[field] = ''
                        logger.info(f"[BUSCAR] Campo {field}: valor='{value}', tipo={type(value)}, procesado='{paciente_data[field]}'")
            ...
            logger.info(f"[ESTADISTICAS] Total pacientes del profesional {profesional_id}: {len(pacientes_profesional)}")
...
def add_professional_patient():
    ...
    try:
        ...
        try:
            ...
            logger.info(f"[OK] Paciente {paciente_id} agregado al profesional {profesional_id}")
        ...
        if 'certificado' in request.files:
            ...
            if file and file.filename:
                if allowed_file(file.filename):
                    ...
                    logger.info(f"[OK] Archivo de certificaci n guardado: {archivo_url}")
                else:
        ...
        for file in files:
            if file and allowed_file(file.filename):
                try:
                    ...
                    logger.info(f"[OK] Archivo {filename} subido y registrado correctamente")
                    
                except Exception as e:
                    logger.error(f"[ERROR] Error procesando archivo {file.filename}: {e}")
                    continue
            else:
                logger.warning(f"[ADVERTENCIA] Archivo no permitido o vac o: {file.filename if file else 'Sin archivo'}")
            ...
            try:
                ...
                for i, record in enumerate(records, start=2):
                    if str(record.get('atencion_id', '')) == str(atencion_id):
                        ...
                        logger.info(f"[OK] Estado de archivos actualizado para atenci n {atencion_id}")
                        break
            except Exception as e:
                logger.warning(f"[ADVERTENCIA] No se pudo actualizar estado de archivos: {e}")
...
def view_archivo(archivo_id):
    ...
    try:
        ...
        if not os.path.exists(file_path):
            ...
            for ruta_alt in rutas_alternativas:
                if os.path.exists(ruta_alt):
                    logger.info(f"[OK] Archivo encontrado en ruta alternativa: {ruta_alt}")
                    file_path = ruta_alt
                    break
            else:
                logger.error(f"[ERROR] Archivo no encontrado en ninguna ruta: {file_path}")
                abort(404)
        ...
        try:
            ...
            if not tiene_acceso:
                logger.warning(f"[ADVERTENCIA] Acceso denegado al archivo {archivo_id} para profesional {profesional_id}")
                abort(403)
...
def test_view_archivo(archivo_id):
    ...
    try:
        ...
        logger.info(f"[BUSCAR] Keys del archivo: {list(archivo.keys()) if archivo else 'None'}")
        
        # Construir la ruta del archivo
        ruta_archivo = archivo.get('ruta_archivo', '')
        logger.info(f"[BUSCAR] ruta_archivo: {ruta_archivo} (tipo: {type(ruta_archivo)})")
        ...
        logger.info(f"[BUSCAR] UPLOAD_FOLDER: {app.config.get('UPLOAD_FOLDER', 'Not set')}")
        ...
        if not os.path.exists(file_path):
            ...
            for ruta_alt in rutas_alternativas:
                ...
                logger.error(f"[ERROR] Rutas probadas: {[file_path] + rutas_alternativas}")
        ...
        try:
            ...
            try:
                ...
                if schedule_data and schedule_data.get('citas'):
                    ...
                    logger.info(f"[OK] Agenda obtenida con m todo optimizado: {len(citas_del_dia)} citas")
                else:
                    # Fallback al m todo original
                    raise Exception("No hay datos en m todo optimizado")
                    
            except Exception as e:
                logger.warning(f"[ADVERTENCIA] M todo optimizado fall , usando fallback: {e}")
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                logger.info("[OK] Hoja Horarios_Profesional creada con horario por defecto")
    ...
    try:
        ...
        try:
            ...
            if recordatorios is not None:
                logger.info(f"[OK] Recordatorios obtenidos con m todo optimizado: {len(recordatorios)} recordatorios")
                return jsonify({
                    'success': True,
...
def update_working_hours():
    ...
    try:
        ...
        # Obtener datos del formulario
        data = request.get_json()
        logger.info(f"[NOTA] Actualizando horarios del profesional {profesional_id}: {data}")
        ...
        try:
            ...
            logger.info(f"[OK] Horarios del profesional {profesional_id} actualizados exitosamente")
...
def database_diagnostic():
    ...
    try:
        ...
        try:
            if sheets_db:
                ...
                try:
                    ...
                    diagnostic_results['sheets_manager']['connection'] = 'OK' if test_result else 'ERROR'
                    ...
                    diagnostic_results['recommendations'].append('Verificar credenciales de Google Sheets')
            else:
                diagnostic_results['sheets_manager']['status'] = 'NO DISPONIBLE'
                diagnostic_results['recommendations'].append('Inicializar SheetsManager')
        except Exception as e:
        ...
        try:
            if auth_manager:
                ...
                try:
                    ...
                    diagnostic_results['auth_manager']['connection'] = 'OK' if test_users is not None else 'ERROR'
                    ...
                    diagnostic_results['recommendations'].append('Verificar configuración de AuthManager')
            else:
        ...
        for var in critical_vars:
            ...
            if not value:
                diagnostic_results['recommendations'].append(f'Configurar variable de entorno: {var}')
        ...
        try:
            if sheets_db:
                ...
                if spreadsheet:
                    ...
                    diagnostic_results['sheets_manager']['spreadsheet_id'] = spreadsheet.id
                else:
                    diagnostic_results['connection_tests']['sheets_api'] = 'FALLÓ'
                    diagnostic_results['recommendations'].append('Verificar ID de Google Sheets')
            else:
                diagnostic_results['connection_tests']['sheets_api'] = 'NO DISPONIBLE'
        except Exception as e:
            diagnostic_results['connection_tests']['sheets_api'] = 'FALLÓ'
            diagnostic_results['recommendations'].append(f'Error en API de Google Sheets: {str(e)}')
        ...
        try:
            if sheets_db:
                ...
                if spreadsheet:
                    ...
                    critical_sheets = ['Usuarios', 'Pacientes', 'Atenciones', 'Citas_Agenda']
                    available_sheets = [ws.title for ws in spreadsheet.worksheets()]
                    
                    diagnostic_results['sheets_manager']['available_sheets'] = available_sheets
                    diagnostic_results['sheets_manager']['critical_sheets_status'] = {}
                    
                    for sheet in critical_sheets:
                        if sheet in available_sheets:
                            diagnostic_results['sheets_manager']['critical_sheets_status'][sheet] = 'DISPONIBLE'
                        else:
                            diagnostic_results['sheets_manager']['critical_sheets_status'][sheet] = 'NO ENCONTRADA'
                            diagnostic_results['recommendations'].append(f'Crear hoja: {sheet}')
        except Exception as e:
            diagnostic_results['sheets_manager']['sheets_check_error'] = str(e)

        # 6. Generar resumen
        total_tests = len(diagnostic_results['connection_tests'])
        passed_tests = sum(1 for test in diagnostic_results['connection_tests'].values() if test == 'PASÓ')
...
def analyze_enhanced():
    ...
    try:
        if not COPILOT_HEALTH_AVAILABLE:
            return jsonify({
                'success': False,
                'message': 'Módulo Copilot Health no disponible'
            }), 503
...
def identify_keywords():
    ...
    try:
        ...
        region_anatomica = clinical_analyzer.identificar_region_anatomica(motivo_consulta)
        ...
        escalas = clinical_analyzer.identificar_escalas_evaluacion(palabras_clave, region_anatomica)
...
def search_enhanced():
    ...
    try:
        ...
        return jsonify({
            ...
            'confianza_global': resultado.confianza_global,
            'resumen_analisis': {
                'palabras_clave_identificadas': len(resultado.analisis_clinico.palabras_clave),
                'patologias_sugeridas': len(resultado.analisis_clinico.patologias_identificadas),
                'escalas_recomendadas': len(resultado.analisis_clinico.escalas_recomendadas),
...
def analyze_motivo_consulta():
    ...
    try:
        if not COPILOT_HEALTH_AVAILABLE:
            return jsonify({
                'success': False,
                'message': 'M dulo Copilot Health no disponible'
            }), 503
        ...
        motivo_analizado = copilot_health.analizar_motivo_consulta(motivo_texto, tipo_atencion)
...
def evaluate_antecedentes():
    ...
    try:
        ...
        if not data or 'antecedentes' not in data:
            return jsonify({
                'success': False,
                'message': 'Antecedentes requeridos'
            }), 400
        ...
        evaluacion = copilot_health.evaluar_antecedentes(antecedentes, especialidad, edad)
...
def suggest_treatment():
    ...
    try:
        ...
        try:
            ...
            for i, tratamiento in enumerate(todos_tratamientos[:10], 1):  # Limitar a 10 resultados
                plan = {
                    ...
                    'evidencia_cientifica': f"Estudio de {', '.join(tratamiento.autores[:2])} ({tratamiento.fecha_publicacion[:4]})" if tratamiento.autores else "Evidencia cient fica",
            ...
            logger.info(f"[OK] Encontrados {len(planes_tratamiento)} tratamientos con t rminos MeSH espec ficos")
...
def complete_analysis():
    ...
    try:
        ...
        evaluacion = copilot_health.evaluar_antecedentes(antecedentes, especialidad, edad)
        planes = copilot_health.sugerir_planes_tratamiento(diagnostico, especialidad, edad)
        
        # Generar resumen completo
        resumen_completo = copilot_health.generar_resumen_ia(motivo_analizado, evaluacion, planes)
...
def generate_evaluation_questions():
    ...
    try:
        ...
        logger.info(f"[BUSCAR] Generando preguntas de evaluaci n para: {motivo_consulta} en {tipo_atencion}")
        ...
        try:
            ...
            preguntas = apis.generar_preguntas_personalizadas_evaluacion(motivo_consulta, tipo_atencion)
            
            logger.info(f"[OK] Generadas {len(preguntas)} preguntas personalizadas para {tipo_atencion}")
...
def planificacion_completa():
    ...
    try:
        ...
        if not data or 'motivo_atencion' not in data:
            return jsonify({
                'success': False,
                'message': 'Motivo de atenci n requerido'
            }), 400
...
def generate_search_terms():
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'terminos_disponibles': terminos_disponibles
        })
...
def extract_key_terms():
    ...
    try:
        ...
        return jsonify({
            'success': False,
            'message': f'Error extrayendo términos clave: {str(e)}'
        })
...
def search_with_key_terms():
    ...
    try:
        ...
        if not terminos_clave:
            return jsonify({'success': False, 'message': 'Términos clave son requeridos'})
        ...
        logger.info(f"✅ Búsqueda con términos clave completada: {len(planes_tratamiento)} resultados")
...
def guardar_sesion():
    ...
    try:
        ...
        # Validar datos requeridos
        required_fields = ['atencion_id', 'fecha_sesion', 'duracion', 'tipo_sesion', 
                         'objetivos', 'actividades', 'progreso', 'estado']
        ...
        if len(sesiones_existentes) >= 15:
            return jsonify({'success': False, 'message': 'Límite máximo de 15 sesiones alcanzado'}), 400
        ...
        return jsonify({'success': True, 'message': 'Sesión registrada exitosamente', 'sesion_id': nueva_sesion['id']})
...
def eliminar_sesion_sheets(sesion_id):
    ...
    try:
        ...
        if row_number:
            ...
            sheets_client.batchUpdate(spreadsheetId=SPREADSHEET_ID, body=request_body).execute()
