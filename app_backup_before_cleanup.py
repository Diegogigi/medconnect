...
# === ENDPOINTS FUNCIONALES PARA DASHBOARD PROFESIONAL ===

@app.route("/api/professional/patients", methods=["GET"])
def get_patients_working():
    """Obtener pacientes - Versión funcional"""
    try:
        if "user_id" not in session:
            return jsonify({"error": "No autenticado"}), 401
        
        # Datos de ejemplo que funcionan
        patients = [
            {
                "id": 1,
                "nombre": "María",
...
def get_atenciones_working():
    """Obtener atenciones - Versión funcional"""
    try:
        if "user_id" not in session:
            return jsonify({"error": "No autenticado"}), 401
        
        # Atenciones de ejemplo que funcionan
        atenciones = [
            {
                "id": 1,
...
def get_schedule_working():
    """Obtener horario - Versión funcional"""
    try:
        if "user_id" not in session:
            return jsonify({"error": "No autenticado"}), 401
        ...
        # Horario de ejemplo que funciona
        schedule = [
            {
                "id": 1,
                "fecha": fecha,
