...
from flask import (
    ...
    url_for, session, make_response, send_from_directory
)
...
def cache_set(key, val): 
    with _cache_lock: _cache[key] = (val, time.time())
...
def diagnose_login_error(err: Exception) -> dict:
    ...
    if "Invalid salt" in msg or "hash" in msg.lower():
        tips.insert(0, "Parece un conflicto de formatos de hash (e.g. werkzeug vs. passlib).")
    return {
...
def allowed_file(filename: str) -> bool:
    ALLOWED = {"png","jpg","jpeg","gif","pdf","doc","docx","bmp","tiff","tif","txt","dcm","dicom"}
...
def detect_intent(text: str) -> str:
    t = (text or "").lower()
    if any(k in t for k in ["emergencia", "auxilio", "ambulancia"]): return "emergencia"
    if any(k in t for k in ["ayuda", "help", "/help"]): return "ayuda"
    return "general"

def get_telegram_user_info(telegram_user_id):
    try:
        return auth_manager.get_user_by_telegram_id(telegram_user_id) if auth_manager else None
    except Exception as e:
...
def handle_patient_requests(text, user_info, user_id, intent):
    if intent == "ayuda":
        return "Puedo registrar consultas, medicamentos, exámenes, y avisar a tu familia."
...
def login():
    if request.method == "GET":
        return render_template("login.html") if os.path.exists(os.path.join("templates","login.html")) else "Login"
    ...
    try:
        ...
        if not ok:
            return render_template("login.html", message="Credenciales inválidas", success=False) \
                if os.path.exists(os.path.join("templates","login.html")) else (jsonify({"error":"Credenciales inválidas"}), 401)
        # set session
        session.clear()
        ...
        return render_template("login.html", message=diag["user_message"], success=False) \
            if os.path.exists(os.path.join("templates","login.html")) else (jsonify({"error":diag["user_message"]}), 500)
...
def patient_dashboard():
    return render_template("patient_dashboard.html") if os.path.exists(os.path.join("templates","patient_dashboard.html")) else "Paciente"

@app.route("/professional")
@login_required
def professional_dashboard():
    # Si tienes profesional_id en session, podrías cargar agenda
    return render_template("professional_dashboard.html") if os.path.exists(os.path.join("templates","professional_dashboard.html")) else "Profesional"
...
def update_medical_info():
    try:
        ...
        logger.info(f"[PROFILE] medical info {session.get('user_id')}: {json.dumps(data)[:200]}")
...
def update_notification_settings():
    try:
        data = request.get_json() or {}
        logger.info(f"[PROFILE] notifications {session.get('user_id')}: {json.dumps(data)[:200]}")
        return jsonify({"success":True})
    except Exception as e:
...
def bot_stats():
    try:
        ...
        return jsonify({"status":"success","stats":stats,"bot_username":"@Medconn_bot"})
    except Exception as e:
        return jsonify({"status":"error","error":str(e),"bot_username":"@Medconn_bot"})
...
def health():
    try:
        return jsonify({
            ...
            "postgres_connected": getattr(postgres_db, "is_connected", lambda: False)()
        })
    except Exception as e:
...
def debug_static():
    ...
    return jsonify({"static_root": static_dir, "count": len(files), "files": files[:50]})

# ---------- ERROR HANDLERS ----------
@app.errorhandler(404)
def not_found(e):
    return (render_template("404.html"), 404) if os.path.exists(os.path.join("templates","404.html")) else ("404", 404)

@app.errorhandler(500)
def internal_error(e):
    logger.error(f"500: {e}")
    return (render_template("500.html"), 500) if os.path.exists(os.path.join("templates","500.html")) else ("500", 500)
