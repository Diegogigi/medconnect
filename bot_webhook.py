#!/usr/bin/env python3
"""
Bot Webhook de MedConnect - Sin conflictos 409
"""
...
# Crear Flask app para webhook
app = Flask(__name__)

class WebhookMedConnectBot:
    def __init__(self):
        ...
        logger.info("✅ Bot webhook iniciado")
    ...
    def setup_webhook(self):
        ...
        try:
            ...
            if not railway_domain:
                # Intentar obtener de otras variables
                railway_domain = os.environ.get('RAILWAY_PUBLIC_DOMAIN', '')
            
            if not railway_domain:
                logger.error("❌ No se pudo obtener el dominio de Railway")
                return False
            ...
            if response.status_code == 200:
                ...
                logger.error(f"❌ Error HTTP configurando webhook: {response.status_code}")
                return False
...
🤖 <b>Bot:</b> Funcionando correctamente (Webhook)
...
def health():
    ...
    return jsonify({
        'status': 'healthy',
        'bot': 'running',
        'database': 'connected' if bot.db else 'disconnected',
...
def setup_webhook():
    ...
    try:
        if bot.setup_webhook():
            return jsonify({'status': 'success', 'message': 'Webhook configurado correctamente'})
        else:
            return jsonify({'status': 'error', 'message': 'Error configurando webhook'}), 500
    except Exception as e:
        return jsonify({'status': 'error', 'message': str(e)}), 500
