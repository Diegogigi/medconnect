...
def register():
    ...
    if request.method == 'POST':
        try:
            ...
            if success:
                logger.info(f"[OK] Usuario registrado exitosamente: {user_data['email']}")
                return render_template('register.html', 
                                     message=message, 
                                     success=True)
            else:
...
def register():
    ...
    if request.method == 'POST':
        try:
            ...
            direccion = request.form.get('direccion', '').strip()
            ciudad = request.form.get('ciudad', '').strip()
            tipo_usuario = request.form.get('tipo_usuario', 'paciente')
            
            # Campos espec ficos para profesionales
            profesion = request.form.get('profesion', '').strip() if tipo_usuario == 'profesional' else ''
            registro_profesional = request.form.get('registro_profesional', '').strip() if tipo_usuario == 'profesional' else ''
            institucion = request.form.get('institucion', '').strip() if tipo_usuario == 'profesional' else ''
            especialidad = request.form.get('especialidad', '').strip() if tipo_usuario == 'profesional' else ''
            ...
            if tipo_usuario == 'profesional' and not all([profesion, registro_profesional]):
                return render_template('register.html', 
                                     message='Los campos de profesi n y registro profesional son obligatorios para profesionales', 
                                     success=False)
...
def get_gendered_profession(profesion, genero=None, nombre=None):
    ...
    logger.info(f"[BUSCAR] Usando g nero normalizado: {genero} para profesi n: {profesion}")
