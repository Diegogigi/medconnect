#!/usr/bin/env python3
"""
Bot de Instancia Única de MedConnect - Evita conflictos 409
"""
...
class SingleInstanceMedConnectBot:
    def __init__(self):
        ...
        if not self.acquire_lock():
            logger.error("❌ Ya hay otra instancia del bot ejecutándose")
            exit(1)
        ...
        logger.info("✅ Bot de instancia única iniciado")
    
    def acquire_lock(self):
        """Adquiere un bloqueo de archivo para evitar múltiples instancias"""
        try:
            lock_file_path = "/tmp/medconnect_bot.lock"
            self.lock_fd = open(lock_file_path, 'w')
            fcntl.flock(self.lock_fd.fileno(), fcntl.LOCK_EX | fcntl.LOCK_NB)
            logger.info("✅ Bloqueo de instancia única adquirido")
            return True
        except (IOError, OSError):
            logger.error("❌ No se pudo adquirir bloqueo - otra instancia en ejecución")
            return False
    ...
    def process_message(self, message):
        """Procesa mensajes con reconocimiento de usuarios"""
        try:
...
¿En qué puedo ayudarte? 🤔"""
    ...
    def get_updates(self):
        """Obtiene actualizaciones del bot con manejo de errores mejorado"""
        try:
            ...
            if response.status_code == 409:
                logger.warning("⚠️ Error 409 - Conflicto de instancias. Reintentando en 10 segundos...")
                time.sleep(10)
                return []
            ...
            logger.warning("⏰ Timeout en getUpdates, reintentando...")
            return []
        except Exception as e:
    ...
    def run(self):
        ...
        logger.info("🚀 Bot de instancia única iniciado y ejecutándose...")
            ...
            while True:
                try:
                    ...
                    for update in updates:
                        ...
                        # Limpiar updates antiguos para evitar memoria infinita
...
if __name__ == "__main__":
    bot = SingleInstanceMedConnectBot()
    bot.run() 